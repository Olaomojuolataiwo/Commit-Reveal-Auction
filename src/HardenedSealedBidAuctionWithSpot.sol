...
///      at reveal cannot influence selection. Commit semantics otherwise unchanged.
contract HardenedSealedBidAuctionWithSpot is ReentrancyGuard {
    ...
    /// @param _priceSource price oracle address (returns price scaled by 1e18)
    constructor(
        address _creator,
    ...
    function commit(bytes32 _commitment) external payable onlyDuringCommit {
        ...
        // Snapshot price at commit time (secure: prevents single-swap manipulation at reveal)
        uint256 snapshot = IPriceOracle(priceSource).getPrice();
        priceSnapshotAtCommit[msg.sender] = snapshot;
    ...
    // ----------------------
    // Reveal phase (uses commit-time snapshot)
    // ----------------------
    /// @notice Reveal bid and supply bid funds. EffectiveBid is computed using price snapshot taken at commit.
    ...
    function reveal(uint256 amount, bytes32 nonce) external payable onlyDuringReveal nonReentrant {
        ...
        // Retrieve commit-time snapshot; ensure snapshot exists
        uint256 snapshot = priceSnapshotAtCommit[msg.sender];
        require(snapshot != 0, "no price snapshot");

        // Compute effective metric using snapshot (normalized to 1e18)
        uint256 effective = mulDiv(amount, snapshot, 1e18);
        ...
        if (effective > winningEffectiveBid) {
            ...
            winningBid = amount; // nominal winning bid for payout
            winner = msg.sender;
    ...
    // Internal utilities
    // ----------------------
    /// @dev simple mulDiv (suitable for typical test values)

...
def main():
    ...
    bid_amount = w3.to_wei(0.01, "ether") # change as desired
    ...
    deposit_amount = w3.to_wei(0.001, "ether") # must match deployed contract's depositAmount

    # Commit to vulnerable auction
    # The deposit is sent via the 'value' field.
    signed_commit_vuln = build_and_sign_tx(vuln.functions.commit(commit_hash), alice, value=deposit_amount)
    ...
    # The deposit is sent via the 'value' field.
    signed_commit_hard = build_and_sign_tx(hard.functions.commit(commit_hash), alice, value=deposit_amount)
    ...
    # 3)---------- Sandwich: build front, reveal, back (single unified block) ----------
    # Build values
    PHIGH = w3.to_wei(0.04, "ether") if price_oracle else None
    ...
    # The full bid amount is sent via 'value' here.
    reveal_signed = build_and_sign_tx(vuln.functions.reveal(bid_amount, nonce), alice, value=bid_amount, nonce=alice_nonce)

    # 4) Send bundle via Flashbots or broadcast sequentially
    ...
    def try_flashbots_bundle(bundle_bytes_list, attempt_blocks=3, offset=1):
		...
		print("  Flashbots send returned. Waiting for inclusion...")
            # The result object returned by the helper has the .wait() method (from the web3-flashbots lib)
    ...
    # bundle not included. If still in reveal phase, broadcast public reveal (freshly signed)
        ...
        # Get fresh nonce for Alice in case it changed
            ...
            try:
                # Value must be included here for the public reveal
